...
def save_LACBED(v):
    ...
    for i in range(v.lacbed_sim.shape[3]):
        ...
        data = v.lacbed_sim[j, :, :, i].astype(np.float32)

        # Save TIFF
        tiff_fname = f"{v.chemical_formula_sum}_{signed_str}.tiff"
        plt.imsave(tiff_fname, data, cmap='gray')

        # Save NumPy
        npy_fname = f"{v.chemical_formula_sum}_{signed_str}.npy"
